...
- 流水线成功后读取 `CHATBI_SAMPLE_DATA_PATH` 指定的路线文件，并在写入数据库前逐条执行结构校验和数据库
  基础数值校验。`hiking_minutes <= 0` 等不合格记录须输出路线 ID、序号和原因后跳过，其余合格路线继续
  增量导入 `CHATBI_DB_PATH`；文件根节点损坏或数据库本身导入失败时本次任务失败。
