...
- 可选使用和风天气每日天气预报 API 获取出发日期的温度和简单天气现象，作为参考信息。
- 预警查询只使用路线起点经纬度，不代表整条路线沿途情况。
- 不为墨脱等特定地区增加单独的天气获取或坐标转换逻辑；第三方数据源可能无法返回部分地区的数据。
- 不提供逐小时天气和系统计算的天气风险，不保存历史预警或天气预报，不新增数据库表，不接入地质灾害、封路等信息。
...
Provider 同时使用 `GET /v7/weather/{days}?location={longitude},{latitude}` 查询每日天气预报，
并按出发日期动态选择最小可覆盖窗口：`3d`、`7d`、`10d`、`15d` 或 `30d`。匹配
`departure_at` 的出发日期后返回最低/最高温、白天/夜间天气现象、降水量、湿度和白天风向风力。
每日天气只作为参考，不参与路线过滤或评分。

和风天气只提供当前生效预警聚合和未来最多三十日天气参考，不提供逐小时天气。返回结果可能因地区覆盖、
...
- `daily_weather`：出发日期的温度和简单天气现象参考；服务未配置、异常或出发日期不在三十日预报范围内时标记为不可用。
...
- 每日天气预报未配置、临时异常或未覆盖出发日期时继续推荐，并在 `daily_weather.fallback_reason` 中说明。
...
- HTTP 接口、推荐结果及 Qwen Agent 工具均能返回官方预警信息和每日天气参考。
